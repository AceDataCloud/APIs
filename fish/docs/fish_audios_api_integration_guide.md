...
For demonstration purposes, you can use a public Webhook sample site https://webhook.site/ to get a Webhook URL. Copy this URL and set it as the `callback_url` field while filling in the corresponding parameters:
...
After a moment, the following result will be posted to the callback URL:
...
The result contains a `task_id` field, and the other fields are similar to those mentioned above, allowing the task to be associated through this field.
