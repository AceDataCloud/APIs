...
> For creating voices, please refer to [Fish Model Create API](https://platform.acedata.cloud/documents/fish-model). To query details of a single voice model by ID, see [Fish Model Get API](https://platform.acedata.cloud/documents/fish-model-get).
...
- **Authentication Method**: Uses `Authorization: ******` where the token is the key applied for on this platform.
