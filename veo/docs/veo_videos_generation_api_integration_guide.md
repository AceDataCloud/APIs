...
- `image_urls`: when selecting the image-to-video action `image2video`, it is necessary to upload the reference image links. `veo2-fast` only supports **1 image**, `veo31-fast-ingredients` supports up to **3 images** (multi-image fusion mode), and all other models support up to **2 images** (first and last frame mode).
...
- **Image quantity limit**:
  ...
  - All other models support a maximum of **2 images** input (first and last frame mode).
...
After video generation is complete, you can use the following APIs for further processing:
...
- [Veo Extend API Integration Guide](veo_extend_api_integration_guide.md): Extend the duration of a generated video (supported by veo31 series models only).
- [Veo Reshoot API Integration Guide](veo_reshoot_api_integration_guide.md): Regenerate a video with new camera motion while preserving the scene content.
