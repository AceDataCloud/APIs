...
First, understand the basic usage method, which involves inputting the prompt `prompt`, the generation action `action`, an array of reference images for the first and last frames `image_urls`, and the model `model` to obtain the processed result. You first need to simply pass a field `action`, with the value set to `text2video`. It mainly includes three actions: text-to-video (`text2video`), image-to-video (`image2video`), and get 1080p video (`get1080p`). Then, we also need to input the model `model`, which currently mainly includes `veo31-fast`, `veo3`, `veo31`, `veo3-fast`, and `veo31-fast-ingredients` models, as detailed below:
...
- `model`: the model for generating the video, mainly including `veo31-fast`, `veo3`, `veo31`, `veo3-fast`, and `veo31-fast-ingredients` models.
