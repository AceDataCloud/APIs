...
Render is the lowest-level interface of WebExtrator. If you need **structured** extraction results (article body, product prices, recipe ingredients, etc.), please use [`/webextrator/extract`](webextrator_extract_api_integration_guide.md) — it runs a complete set of typed extraction pipelines based on the same rendering foundation.
...
The result will be pushed to `callback_url` via `POST` (if configured), or can be actively queried through [`/webextrator/tasks`](webextrator_tasks_api_integration_guide.md).
