...
To use the Gemini Videos Generation API, first obtain your API Token from the [Ace Data Cloud Console](https://platform.acedata.cloud/console/applications) for future reference.
...
- `finished_at`: The completion time of the task, Unix timestamp (seconds, float). This field is not returned if the task is not completed.
...
- `ids`: An array of task IDs to be queried.
