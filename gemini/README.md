...
Google Gemini is a powerful AI conversation system that supports both OpenAI-compatible chat completions format and Google's native `generateContent` / `streamGenerateContent` endpoints. The current flagship models are `gemini-3.1-pro-preview` and `gemini-3.6-flash` (Chat Completions only), with `gemini-2.5-pro`, `gemini-2.5-flash`, and other variants also available. Gemini models support multi-modal inputs (text and images), thinking mode, function calling, and JSON mode.

## Application Process

To use the Gemini API, obtain an API token from the [Ace Data Cloud Console](https://platform.acedata.cloud/console/applications). One API token can call all services on the platform.
