...
When using this interface for the first time, we need to fill in at least three pieces of content: one is `authorization`, which can be selected directly from the dropdown list. The other parameter is `model`, which is the category of the Grok official model we choose to use. Currently available models are `grok-4` and `grok-3`. The last parameter is `messages`, which is an array of our input questions. It is an array that allows multiple questions to be uploaded simultaneously, with each question containing `role` and `content`. The `role` indicates the role of the questioner, and we provide three identities: `user`, `assistant`, and `system`. The other `content` is the specific content of our question.
...
- `reasoning_effort`: Controls the reasoning depth for thinking models. Accepts `minimal`, `low`, `medium`, or `high` (default: `medium`).
- `web_search_options`: Enables web search augmentation for the response. Use `search_context_size` (`low`, `medium`, `high`) to control context volume.
