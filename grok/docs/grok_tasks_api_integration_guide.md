...
The Grok Tasks API lets you query async Grok tasks after the original request returns a `task_id`.
Use it to retrieve one task by `id` or fetch multiple tasks in a single batch request.

## Endpoint

```text
POST https://api.acedata.cloud/grok/tasks
```
...
```bash
curl -X POST 'https://api.acedata.cloud/grok/tasks' -H 'accept: application/json' -H 'authorization: ******' -H 'content-type: application/json' -d '{
