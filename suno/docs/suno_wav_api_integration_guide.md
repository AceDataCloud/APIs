...
>
> WAV files on the upstream Suno CDN (`https://cdn1.suno.ai/{audio_id}.wav`) are only kept for a few days before being reclaimed (returning 403 on access).
> To prevent broken links, this API automatically transfers the upstream WAV file to our own CDN (`https://platform.cdn.acedata.cloud/suno/{task_id}.wav`) before returning it. The returned `file_url` is this stable, transferred address and will not expire when the upstream CDN link does.
> In the rare case of a transfer failure, the API falls back to the original upstream URL, maintaining backward-compatible behavior.