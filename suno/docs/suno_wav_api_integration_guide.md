...
> **About WAV link persistence**
>
> WAV files on the upstream Suno CDN (`https://cdn1.suno.ai/{audio_id}.wav`) are only retained for a few days before being recycled and returning 403 on access.
> To prevent links from expiring, this API automatically transfers the upstream WAV file to our own CDN (`https://platform.cdn.acedata.cloud/suno/{task_id}.wav`) before returning. The `file_url` returned is the stable transferred address and will not expire due to upstream CDN expiry.
> In the rare case that the transfer fails, it falls back to the original upstream URL, maintaining backward-compatible behavior.