...
>
> WAV files on the upstream Suno CDN (`https://cdn1.suno.ai/{audio_id}.wav`) are only retained for a few days before being recycled and returning 403.
> To prevent link expiry, this API automatically transfers the upstream WAV file to our own CDN (`https://platform.cdn.acedata.cloud/suno/{task_id}.wav`) before returning, so the `file_url` returned is the stable transferred address and will not expire immediately when the upstream CDN expires.
> **The transferred link is valid for 30 days**, after which the file will be periodically cleaned up. It is recommended to download and save the file to your own storage as soon as possible.
> In the rare case of a transfer failure, it falls back to the original upstream URL, maintaining the same behavior as before.