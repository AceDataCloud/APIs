...
Enhanced uploads are processed asynchronously and typically take 2 minutes or longer. Successful processing consumes **1.87 Credits**, which is approximately **¥1.26 per upload** based on the maximum package; failed processing incurs no charges.
...
The interface immediately returns `task_id` and `trace_id`. Use the [Suno Tasks API](https://platform.acedata.cloud/documents/suno-tasks) to query the task, and upon success, read the uploaded audio ID from `response.data.audio_id`. You can also provide an HTTPS `callback_url` to receive the final state.
