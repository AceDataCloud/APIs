...
- `audio_urls`: a list of reference audio URLs (1 to 4 publicly accessible audio addresses); used with the `inspo` action.
...
Suno's Inspo feature allows you to generate brand-new music inspired by 1 to 4 reference audio clips. Unlike a cover, Inspo does not replicate the original track; instead, it extracts style inspiration from the reference audio and creates a new composition based on optional prompts and style tags. You need to fill in the following parameters:

- action: The content is `inspo`.
- audio_urls: A list of reference audio URLs (1 to 4 publicly accessible audio addresses).
- model: The model to use, for example `chirp-v5`.
- prompt: Lyrics or creative prompt (optional).
- style: Music style tags, for example `acoustic, folk, warm` (optional).
- title: Song title (optional).
- audio_weight: The influence weight of the reference audio on the generated result, range 0 to 1 (optional).

> Note: Reference audio must be publicly accessible. If the reference audio exactly matches a known recording in Suno's library, Suno may reject generation due to copyright verification. It is recommended to use your own audio or audio generated by Suno as the inspiration source.
...
Clicking run, you can find that a result is returned, as follows:
...
This completes the Inspo feature operation, and the returned result is consistent with ordinary song generation.
