...
The Kling Lip Sync API drives an existing Kling video with audio or text so the character speaks in sync, enabling talking-photo and digital-human narration pipelines.

The Kling Talking Photo API turns a portrait image and an audio clip into a talking video in a single call by internally chaining image-to-video animation and lip sync.
