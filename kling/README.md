...
The Kling Lip Sync API drives an existing Kling video with audio or text so the character speaks in sync (lip sync).

The Kling Talking Photo API turns a portrait image and an audio file into a talking video in a single call, internally combining image-to-video animation and lip sync.
