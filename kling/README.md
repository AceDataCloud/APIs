...
Kling AI video generation service with text-to-video, image-to-video, motion control, lip sync, talking photo, and video extension.
...
The Kling Lip Sync API drives an existing Kling video with audio or text so the character speaks in sync.

The Kling Talking Photo API generates a talking video in one call using one portrait image and one audio file.
