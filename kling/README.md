...
Kling AI video generation service with text-to-video, image-to-video, motion control, lip sync, talking-photo animation, and video extension.
...
The Kling Motion Generation API generates videos with character motion driven by a reference image and a reference video, keeping character movements consistent with the reference video. Kling also supports dedicated Lip Sync and Talking Photo endpoints for speech-driven animation workflows.
...
| Kling Lip Sync API | `/kling/lip-sync` | Generate lip-synced video clips from source media and driving audio. |
| Kling Talking Photo API | `/kling/talking-photo` | Animate a still portrait into a speaking video clip. |
