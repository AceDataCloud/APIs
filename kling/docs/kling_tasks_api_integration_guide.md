...
To use the Kling Tasks API, you first need to apply for the corresponding service on the application page [Kling Videos Generation API](https://platform.acedata.cloud/documents/3b921a16-a411-4557-8335-53f21d3f9e46), and then copy the task ID from the Kling Videos Generation API response, as shown in the image:

<p><img src="https://cdn.acedata.cloud/om6p6g.png" width="500" class="m-auto"></p>

Finally, go to the Tasks API page [Kling Tasks API](https://platform.acedata.cloud/documents/6b43ff7b-8298-49c8-a390-83df85c2c5e5) to apply for the corresponding service. After entering the page, click the "Acquire" button, as shown in the image:
...
The Kling Tasks API can be used to query the results of the Kling Videos Generation API. For information on how to use the Kling Videos Generation API, please refer to the documentation [Kling Videos Generation API](https://platform.acedata.cloud/documents/3b921a16-a411-4557-8335-53f21d3f9e46).

We will use one task ID returned by the Kling Videos Generation API as an example to demonstrate how to use this API. Suppose we have a task ID: `20068983-0cc9-4c6a-aeb6-9c6a3c668be0`. Below is a demonstration of how to query by passing in a single task ID.
...
- `action`: The operation type for the task.
...
The returned result contains multiple fields. The `items` field contains the specific details of all batch video tasks. Each video task's detail format is the same as the single task query response. The field descriptions are as follows:

- `items`: All specific details of the batch video tasks. It is an array, with each element having the same format as the single task query response.
...
Through this document, you have learned how to use the Kling Tasks API to query single or batch video task details. We hope this document helps you integrate and use the API more effectively. If you have any questions, please feel free to contact our technical support team.
