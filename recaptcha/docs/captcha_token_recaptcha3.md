...
| `proxy` | string | ❌ | Optional proxy. Format: `scheme://[user:pass@]host:port`. Supports `http`/`https`/`socks4`/`socks5`. |
...
**Billing:** Creating a task and polling while `processing` are free. A charge is incurred only when the token is successfully returned.
