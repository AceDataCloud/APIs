...
| Field | Description |
| --- | --- |
| `model` | `gpt-image-2`, `gpt-image-2.5-flare` (faster), or `gpt-image-2.5-sunburst` (higher fidelity and control); all three support their corresponding `:official` variants, and `gpt-image-2` also supports `:reverse` |
...
`gpt-image-2`, `gpt-image-2.5-flare`, `gpt-image-2.5-sunburst`, and `gpt-image-2:reverse` are billed based on the number of successful images; `gpt-image-2:official`, `gpt-image-2.5-flare:official`, and `gpt-image-2.5-sunburst:official` are settled based on the actual tokens of text input, reference image input, and image output, with the final billing based on usage records.
