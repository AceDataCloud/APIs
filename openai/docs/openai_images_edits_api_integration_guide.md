...
OpenAI image editing service allows you to input images and instructions, and outputs the edited images. The GPT Image series models can accept up to 16 reference images simultaneously. Currently, the API supports `gpt-image-1`, the latest **`gpt-image-2`**, as well as the **`nano-banana` / `nano-banana-2-lite` / `nano-banana-2` / `nano-banana-pro`** series models accessed through the same interface.
