...
- **Supports high-resolution redrawing**: You can pass a 1K source image and request 2K / 4K output via the `size` parameter; the model will upscale while applying the edit.

### Supported `size` Values and Billing Tiers

The `size` constraints for the edit endpoint are identical to those of the generation endpoint — `gpt-image-2` only accepts the 15 sizes listed below; any other value will return a 400 error. **The billing for 2K / 4K tiers is 1.5× the 1K tier rate**, regardless of the source image resolution — only the requested `size` value matters:
...
> For example: if the source image is `1024x1024` and `size` is set to `2048x2048`, the model will redraw at 2K and bill at the 2K rate; setting `size` to `3840x2160` outputs a 4K landscape image billed at the 4K rate.
>
> You can also pass `size: "auto"` or **omit the `size` field**, in which case the model selects a default size and billing uses the 1K rate.
