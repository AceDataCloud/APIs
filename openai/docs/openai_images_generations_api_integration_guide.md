...
- **Native multi-aspect-ratio and high-resolution support**: Covers 5 aspect ratios (1:1, 4:3, 3:4, 16:9, 9:16) across 3 resolution tiers (1K / 2K / 4K).
...
`gpt-image-2` only accepts the 15 sizes listed in the table below; any other value will return a 400 error. **The billing for 2K / 4K tiers is 1.5× the 1K tier rate**:
...
> You can also pass `size: "auto"` or **omit the `size` field** entirely; in that case the model selects a default size and billing uses the 1K rate.
>
> Actual billing is `n × per-image cost`; a 4K request typically takes 4–8 minutes, so pairing it with the `callback_url` async callback described later is recommended.
