...
- **1K standard rate**: The requested size is one of the 1K recommended sizes in the table below, or one of the upstream 1K output aliases (`1254x1254`, `1672x941`, `941x1672`). These are the actual dimensions the upstream model returns at the 1K tier; using them as the `size` parameter value will not trigger a higher tier.
...
>
> 1K-tier output may not match the requested dimensions exactly — passing `1024x1024` may yield `1254x1254`; aspect ratio is preserved. If you use that returned size as the `size` parameter value in a subsequent request, it is still billed at the 1K rate.
>
