...
When using this interface for the first time, we need to fill in at least three pieces of information: one is `authorization`, which can be selected directly from the dropdown list. The other parameter is `model`, which is the category of the OpenAI ChatGPT model we choose to use. Available models include the latest `gpt-5.6-luna`, `gpt-5.6-terra`, `gpt-5.6-sol` series, the `gpt-5` / `gpt-5.5` series, the `gpt-4.1` / `gpt-4o` series, reasoning models `o1`, `o3`, `o4-mini`, as well as free-tier variants (`gpt-oss:free`, `gpt-5.5:free`, `gpt-5:free`, `gpt-4.1:free`, `gpt-4o:free`, `gpt-4o-mini:free`); details can be found in the models we provide. The last parameter is `messages`, which is an array of our input questions. It is an array that allows multiple questions to be uploaded simultaneously, with each question containing `role` and `content`. The `role` indicates the role of the questioner, and we provide three identities: `user`, `assistant`, and `system`. The other `content` is the specific content of our question.
