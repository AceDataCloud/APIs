...
- `country`: the country where the search results are located.
- `language`: the language of the search results.
- `range`: the time range for the search results.
...
- `image_size`: an image size filter (`large`, `medium`, `icon`, or a megapixel value such as `2mp`); only valid when `type` is `images`.
