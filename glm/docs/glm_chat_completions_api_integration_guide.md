...
- `model`: Choose the GLM model to call, currently supported models include:
  - `glm-5.2`: The latest flagship model with the strongest overall capability.
...
To return responses in a streaming manner, set the `stream` parameter in the request body to `true`.
...
| `glm-4.6`      | A balanced choice for general dialogue and content creation |
